        ...
        public void getIEP1(out string[] C1, out string[] C2, int StudentId, int SchoolId, string[] args, int pageno)
        {
            try
            {
                ...
                try
                {
                    ...
                    SqlCommand command = new SqlCommand("StudentMoreDetailsNE", sqlConnection);
                    ...
                    adapter.Fill(Dt);
                 //   System.Diagnostics.Debugger.Log(1, "StudentMoreDetailsNE:-Type=" + type, "After SP Call.");
                    sqlConnection.Close();
                    if (pageno == 4)
                    {
                        type = "INEX";
                        sqlConnection = new SqlConnection(Connection);
                        command = new SqlCommand("StudentMoreDetailsNE", sqlConnection);
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.Add("@SchoolId", SqlDbType.Int).Value = SchoolId;
                        command.Parameters.Add("@StudentId", SqlDbType.Int).Value = StudentId;
                        command.Parameters.Add("@Type", SqlDbType.VarChar).Value = type;
               ...
               // System.Diagnostics.Debugger.Log(2, "StudentMoreDetailsNE:-Type=" + type + "IEPC.count=" + IEPC.Length + "IEPCHK.Count=" + IEPCHK.Length, "After SP Call.");
                int index = 0;
                if (pageno == 2)
                {
                    if (Dt.Rows.Count < 4)
              ...
              //  System.Diagnostics.Debugger.Log(3, "StudentMoreDetailsNE:-Type=" + type + "Dt.Rows.Count=" + Dt.Rows.Count, "After Dt rows added.");
                if (pageno == 3)
                {
                    if (Dt.Rows.Count < 3)
                    {
           ...
           //     System.Diagnostics.Debugger.Log(4, "StudentMoreDetailsNE:-Type=" + type + "IEPC.count=" + IEPC.Length + "IEPCHK.Count=" + IEPCHK.Length, "After DT Loop.");
                if (pageno == 4)
                {
                    if (Dt2 != null)
                    {
